...
if(
    ...
    if( 
        ...
        try{
            ...
            $r_calorie = round(($r_calorie + ($i_calorie * $multiplier)), 2);
            $r_fat = round(($r_fat + ($i_fat * $multiplier)), 2);
            $r_protein = round(($r_protein + ($i_protein * $multiplier)), 2);
            $r_carb = round(($r_carb + ($i_carb * $multiplier)), 2);
            $r_price = round(($r_price + (($i_price * $multiplier) * 1.1)), 2);

            $myquerystring="INSERT INTO recipes_ingredients (recipe_id, ingredient_id, amount, unit) 
